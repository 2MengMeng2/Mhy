...
(z_train, z_val, z_test, 
 ...
 Y_train, Y_val, Y_test) = data_split(X[:,:,0], X[:,:,1:], Y, val=val*2, test=test*2)
...
if roc_curve:
    ...
    if plt:
        ...
        masses = np.asarray([ef.ms_from_p4s(ef.p4s_from_pthetaphipids(x).sum(axis=0)) for x in X])
